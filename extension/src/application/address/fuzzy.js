(() => {
  const AddressFuzzy = {
    ...
    findBestMatch(query, candidates, threshold = 0.7) {
      ...
      const prefixRegex = /^(tỉnh|tinh|thành\s*phố|thanh\s*pho|tp\.?|t\.?|quận|quan|huyện|huyen|thị\s*xã|thi\s*xa|q\.?|h\.?|phường|phuong|xã|xa|thị\s*trấn|thi\s*tran|p\.?|x\.?|tt\.?)\s+/i;
      
      const queryClean = removeAccentsFn(query.trim().toLowerCase()).replace(prefixRegex, '').trim();
      
      for (const cand of candidates) {
        const candClean = removeAccentsFn(cand.trim().toLowerCase()).replace(prefixRegex, '').trim();
        if (candClean === queryClean) {
          return { match: cand, score: 1.0 };
        }
        
        if (candClean.length >= 4) {
          if (queryClean.startsWith(candClean) || queryClean.endsWith(candClean)) {
            const lenScore = Math.min(queryClean.length, candClean.length) / Math.max(queryClean.length, candClean.length);
            const bonusScore = Math.max(0.85, lenScore);
