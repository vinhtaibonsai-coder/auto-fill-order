  ...
  export const OrderStorage = {
    ...
    async _getScopedKey(baseKey) {
      ...
      if (baseKey === 'activeShopId' || baseKey === 'activeShop') return 'current_shop_id';
    ...
    async saveSubmittedOrder(order) {
      ...
      if (existing) {
        ...
        if (idx !== -1) {
          const updatedOrder = { ...updatedOrdersList[idx], ...order, id: updatedOrdersList[idx].id };
    ...
    async updateSubmittedOrderTracking(savedOrderId, trackingCode) {
      const allSubmitted = await this._getSubmittedOrdersFromLocal();
      const order = allSubmitted.find(o => o.savedOrderId === savedOrderId || o.id === savedOrderId);
      if (order) {
      ...
      return new Promise(async (resolve) => {
        try {
          ...
          if (user && user.id) {
            ...
            if (this.isExtensionAvailable()) {
              chrome.storage.local.set({ [shopsKey]: updatedList }, () => resolve([userDefaultShop]));
            } else {
              localStorage.setItem(shopsKey, JSON.stringify(updatedList));
    ...
    async getActiveShop() {
      ...
      if (this.isExtensionAvailable()) {
        ...
        activeId = localStorage.getItem(activeShopKey);
      }
