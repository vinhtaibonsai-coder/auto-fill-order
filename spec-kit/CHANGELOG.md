...
### Added
- **Enterprise Multi-Tenant Architecture (V3)**: Cô lập dữ liệu 100% giữa các Shop bằng Supabase Row Level Security (RLS) policies (`v3_enterprise_schema.sql`).
- **Supabase Auth & Session Management**: Tách biệt mô-đun `AuthService`, `AuthSession`, `AuthEvents` và `ProfileService` cho Đăng nhập / Đăng ký bằng Email & Mật khẩu.
- **Multi-Tenant Shop Sharing & Invite System**:
  - Hỗ trợ Đăng ký Tạo Shop Mới (Chủ Shop) hoặc Tham gia Shop đã có qua Mã Mời (Nhân viên).
  - Mô-đun `InviteService` sinh tự động Mã Mời Shop dạng `YENSHOP-A8F2` có thời hạn 7 ngày.
- **Dùng Chung 100% Dữ Liệu Đơn Hàng Theo Shop**: Đơn nháp, Đơn đã lên, Khách hàng, Lịch sử bóc tách và Cấu hình kho người gửi tự động lọc & lưu theo `shop_id`.
- **Per-Shop Realtime Subscription**: Mô-đun `RealtimeService` đăng ký kênh `shop-UUID` đồng bộ dữ liệu tức thì giữa máy tính và ứng dụng di động của tất cả nhân viên.
- **Audit Logs & Activity Feed**: Mô-đun `AuditService` tự động ghi vết thao tác sửa/xóa đơn lên bảng `audit_logs` & `activity_logs` và hiển thị bảng Nhật ký hoạt động Realtime trên giao diện.
- **Thống Kê Báo Cáo Chuyên Sâu theo Đơn Đã Lên**:
  - KPI Đơn đã lên, COD đã thu hộ, tỷ lệ hoàn tất đăng đơn.
  - Bảng Xếp Hạng Khách Hàng chuyên sâu (Top Customer Leaderboard Table) phân hạng VIP, Thân Thiết, Khách Mới kèm nút mở chat Zalo 1-click.
- **Ứng Dụng Mobile Web App (`index.html`)**: Giao diện di động phản hồi mượt mà tích hợp Công cụ Bóc Tách Tin Nhắn Chốt Đơn Tự Động và đẩy đơn nháp Realtime về Extension.
