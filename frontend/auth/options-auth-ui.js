...
async function initAuthUI() {
  ...
  function updateTopbarUserUI(user) {
    if (!topbarUserLabel) return;
    if (!user) { topbarUserLabel.textContent = '👤 Chưa đăng nhập'; return; }
    const name = user.full_name || user.username || user.email;
    AuthService.getUserRole().then(role => {
      const badge = role ? ` (${role})` : '';
      topbarUserLabel.textContent = `👤 ${name}${badge}`;
    }).catch(() => {
      topbarUserLabel.textContent = `👤 ${name}`;
    });
  }
