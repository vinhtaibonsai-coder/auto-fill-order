...
const ADDRESS_SCRIPTS = [
  '../../backend/features/address/database/data.js',
  '../../backend/features/address/database/ward_merger.js',
  '../../backend/features/address/database/data-new-loader.js',
  '../../backend/features/address/fuzzy.js',
  '../../backend/features/address/aliases.js',
  '../../backend/features/address/normalizer.js',
  '../../backend/features/address/validator.js',
  '../../backend/features/address/parser.js',
  '../../backend/features/address/rules.js',
  '../../backend/features/address/learning.js',
  '../../backend/features/address/ai.js',
  '../../backend/features/address/engine.js',
];
...
onDOMReady(() => {
    ...
    window.loadSettingsToUI = async function() {
        if (typeof OrderStorage !== 'undefined') {
            ...
            // P0-04: không điền groqApiKey vào input — được quản lý server-side
            const apiKeyInput = document.getElementById('apiKey');
            if (apiKeyInput) {
              apiKeyInput.value = '✔️ Được quản lý server-side qua AI Gateway';
              apiKeyInput.readOnly = true;
    ...
    if (btnSavePrompt) {
        btnSavePrompt.addEventListener('click', () => {
            const promptVal = customAiPromptInput.value.trim();
    ...
    if (btnResetPrompt) {
        btnResetPrompt.addEventListener('click', () => {
            if (confirm('Bạn có chắc chắn muốn khôi phục Prompt AI về mặc định?')) {
                customAiPromptInput.value = DEFAULT_PROMPT;
                try {
                    if (typeof OrderStorage !== 'undefined') {
    ...
    // 3. QUẢN LÝ BLACKLIST (DANH SÁCH ĐEN)
    ...
    async function loadBlacklist() {
        if (typeof window.loadSettingsToUI === 'function') {
            await window.loadSettingsToUI();
        }
    }
        ...
        filtered.forEach(item => {
            ...
            btnDelete.addEventListener('click', () => {
                if (confirm('Bỏ chặn SĐT ' + item.phone + '?')) {
                    blacklistArray = blacklistArray.filter(b => b.phone !== item.phone);
                    if (typeof OrderStorage !== 'undefined') {
